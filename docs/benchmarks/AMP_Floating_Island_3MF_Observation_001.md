...
| --- | --- |
| Observer implementation commit | `0e92a6e8a6dfd2a159aa01aa3b46e6e045ffe42a` |
...
The observation schema remains `0.1`. This refreshed report records the following schema behavior from implementation commit `0e92a6e8a6dfd2a159aa01aa3b46e6e045ffe42a`:
...
- Each object has ordered `part_assignments` records with part ID, explicit assignment, effective assignment after inheritance, and resolved material. A singular object material summary is retained only when all effective assigned parts agree; objects with no part records retain their object-level assignment.
...
- Plate membership is instance-aware. Plates contain unique sorted `object_ids` and ordered instance records, while objects contain ordered instances plus sorted unique `plate_ids` and `plate_names`. All 27 objects in this package appeared on exactly one plate with one recorded instance.
