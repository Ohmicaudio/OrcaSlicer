...
The official Orca wiki is the primary baseline reference for manual/static mixed nozzle-size support. The first AMP CLI baseline probe confirmed normal toolchanger/U1 control exports but did not produce real mixed-nozzle G-code from scratch CLI profile edits. A follow-up upstream Orca GUI probe with repaired four-region fixture bodies did export mixed-nozzle G-code containing `0.2`, `0.4`, `0.6`, and `0.8` nozzle classes plus real `T0` / `T1` / `T2` / `T3` tool commands. The Orca issues and discussions are project/community reports, not proof that any particular workflow is safe on U1 hardware.
